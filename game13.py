...
pygame.display.set_caption("DarkMyst")
...
restart_img = pygame.image.load('').convert_alpha()

#backgound
pine_img = pygame.image.load('').convert_alpha()# image background
pine2_img = pygame.image.load('').convert_alpha() # image background
mountain_img = pygame.image.load('').convert_alpha() # image backgound mountain
sky_img  = pygame.image.load('').convert_alpha() #image sky image

# sotre tiles in a list
# put the image path()
img_list = []
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self , char_type , x , y , scale , speed , ammo = 20):
            ...
            for x ,tile in enumerate(row):
                if tile >= 0:
                    ...
                    if tile >= 0 and tile <=8:
                        self.obstacle_list.append(title_data)
                        ...
                        exit = KeyBox(img, x * TILE_SIZE, y * TILE_SIZE)
                        exit_group.add(exit)
                    elif tile == 20:
                        exit = Exit(x * TILE_SIZE, y * TILE_SIZE)
                        item_box_group.add(exit)
...
class KeyBox(pygame.sprite.Sprite):
    ...
    def update(self):
        # Check if the player has picked up the key box
        if pygame.sprite.collide_rect(self, player):
            player.has_key = True  # Custom attribute to track key possession
            self.kill()  # Remove the key box from the game
...
class ScreenFade():
    ...
    def fade(self):
        ...
        if self.direction == 2: # vertical screen fad down
...
# create buttons
start_button = button.Button(SCREEN_HEIGHT // 2 -130, SCREEN_HEIGHT// 2 - 150, start_img,1)
exit_button = button.Button(SCREEN_HEIGHT // 2 - 110, SCREEN_HEIGHT // 2+50 , exit_img,1)
...
while run:
    ...
    # update player animation state based on their actions
    if player.alive:
        #fire a bullet if player alive
        if shoot:      
                player.shoot()
        if player.in_air: #mean jump
    ...
    if level_complete:
        ...
        bg_scroll = 0 # reset the 0 start
        world_data = reset_level() # to clear the sll sprite groups
        if level<= MAX_LEVELS:
            #LOAD IN LEVEL DATA  create world
            with open() as csvfile: #f'level{level}_data.csv', newline=''
                reader = csv.reader(csvfile,delimiter= ',')
                for x, row in enumerate(reader):
                    for y , tile in enumerate(row):
                        world_data[x][y] = int(tile)
            
            player = Soldier('player',200,200,3,5) # create the a solder
            health_bar = Healthbar(10,10,player.health,player.health)
    else :
        screen_scroll = 0
        if death_fade.fade():
