# 2 / 05/ 2024 10.00 to 00.05 
#need to check the tute 5
...
class Solder(pygame.sprite.Sprite):
    ...
    def update_animation(self):
        ...
        if self.frame_index >= len(self.animation_list[self.action]):
            if self.action == 3:
                self.frame_index = len(sel.animation_list[self.action]) - 1
            else:    
                self.frame_index = 0
...
enemy = Solder('enemy',400,200,3,5)
...
while run:
    ...
    player.draw()
    # do i need to add the update the code? 
    enemy.draw()
